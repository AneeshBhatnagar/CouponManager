...
public class AddNewCouponFragment extends Fragment {
    ...
    public void onStart() {
        ...
        if(db.getStoreCount()==0){
            Toast.makeText(context,"You need to create a store first!",Toast.LENGTH_LONG).show();
            ...
            fragmentTransaction.replace(R.id.container_body, new AddNewStoreFragment());
            ...
            public void onClick(View v) {
                ...
                if(couponCode.getText().toString().isEmpty() ||
                    ...
                    if(giftCard.isChecked() && value.getText().toString().isEmpty()){
                        ...
                        Coupon c = new Coupon(store_id, type, couponCode.getText().toString(),
                                couponDesc.getText().toString(), expiryDate.getText().toString(),giftValue);
                        db.addCoupon(c);
                        InputMethodManager imm = (InputMethodManager)context.getSystemService(Context.INPUT_METHOD_SERVICE);
                        imm.hideSoftInputFromWindow(view.getWindowToken(), 0);
                        Toast.makeText(context,"Coupon Saved",Toast.LENGTH_LONG).show();
                        FragmentManager fragmentManager = getActivity().getSupportFragmentManager();
                        FragmentTransaction fragmentTransaction = fragmentManager.beginTransaction();
